...
class _Stage0State extends State<Stage0> {
  ...
  void checkResult() {
    if (int.parse(userAnswerX) > 1 && int.parse(userAnswerC) > -10) {
      showDialog(
          context: context,
          builder: (context) {
      ...
      benar();
        ...
        (int.parse(userAnswerC) > 7 || int.parse(userAnswerC) <= 2)) {
      showDialog(
          context: context,
          builder: (context) {
            return ResultMessage(
      ...
      benar();
        ...
        (int.parse(userAnswerC) < 0 || int.parse(userAnswerC) > 7)) {
      showDialog(
          context: context,
          builder: (context) {
            return ResultMessage(
  ...
  void goToNextQuestion() async {
    ...
    if (playerId != "") {
      ...
      if (level <= 3) {
        players.levelup(AuthService().getId(user), 4);
      } else {
        //raono
      }
    } else {
      ...
      await Players().uhuyno(finalDeviceId, 4);
    }
  }
