...
class _Stage0State extends State<Stage0> {
  ...
  void checkResult() {
    if (int.parse(userAnswerX) > 0 && int.parse(userAnswerC) > 2) {
      showDialog(
          context: context,
          builder: (context) {
      ...
      benar();
    } else if (int.parse(userAnswerX) < 0 && int.parse(userAnswerC) < 0) {
      showDialog(
          context: context,
          builder: (context) {
            return ResultMessage(
      ...
      benar();
        ...
        (int.parse(userAnswerC) > 2 || int.parse(userAnswerC) < 0)) {
      showDialog(
          context: context,
          builder: (context) {
            return ResultMessage(
  ...
  void goToNextQuestion() async {
    ...
    if (playerId != "") {
      ...
      if (level <= 2) {
        players.levelup(AuthService().getId(user), 3);
      } else {
        //raono
      }
    } else {
      ...
      await Players().uhuyno(finalDeviceId, 3);
    }
  }
